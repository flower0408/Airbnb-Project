...
func (rr *AppointmentRepo) UpdateAppointment(ctx context.Context, id string, appointment *Appointment, token string) error {
	...
	rr.logger.Infoln("AppointmentRepo.UpdateAppointment : reached InsertAppointment in repo")

	originalAppointment, err := rr.GetAppointmentByID(ctx, id)
	if err != nil {
		rr.logger.Errorf("AppointmentRepo.UpdateAppointment : Error retrieving original appointment")
	...
	if err != nil {
		rr.logger.Errorf("AppointmentRepo.UpdateAppointment : Error sending reservation request")
	...
	if reservationResponse.StatusCode == http.StatusOK {
		...
		if err != nil {
			rr.logger.Errorf("AppointmentRepo.UpdateAppointment : Error getting appointments by accommodation")
			span.SetStatus(codes.Error, "Error getting appointments by accommodation")
			return err
		}
		...
		for _, existingAppointment := range existingAppointments {
			...
			for _, existAppointment := range existingAppointment.Available {
				for _, newAppointment := range appointment.Available {
					...
					if newAppointment.Equal(existAppointment) {
						rr.logger.Errorf("AppointmentRepo.UpdateAppointment : Error editing appointment. Date already exists")
		...
		for _, newAppointment := range appointment.Available {
			rr.logger.Errorf("AppointmentRepo.UpdateAppointment : Error editing appointment. Cannot add appointment in the past.")
		...
		if appointment.PricePerGuest != 0 && appointment.PricePerAccommodation != 0 {
			rr.logger.Errorf("AppointmentRepo.UpdateAppointment : Error adding accommodation price and guest price at the same time.")
			span.SetStatus(codes.Error, "Error adding accommodation price and guest price at the same time.")
			return errors.New("Error adding accommodation price and guest price at the same time.")
		}
		...
		if err != nil {
			rr.logger.Errorf("AppointmentRepo.UpdateAppointment : Error converting ID to ObjectID:")
		...
		if err != nil {
			rr.logger.Errorf("AppointmentRepo.UpdateAppointment : Error %s", err)
			span.SetStatus(codes.Error, err.Error())
			return err
		}
	} else if reservationResponse.StatusCode == http.StatusBadRequest {
		rr.logger.Errorf("AppointmentRepo.UpdateAppointment : Reservation exists for the appointment. Update not allowed.")
	...
	rr.logger.Infof("AppointmentRepo.UpdateAppointment : Data of updated appointment: %+v", appointment)
	rr.logger.Infoln("AppointmentRepo.UpdateAppointment : UpdateAppointment success")
	return nil
}
...
func (rr *AppointmentRepo) FilterAppointmentsByPrice(ctx context.Context, minPrice, maxPrice int) (Appointments, error) {
	...
	rr.logger.Infoln("AppointmentRepo.FilterAppointmentsByPrice : reached FilterAppointmentsByPrice in repo")
	...
	if err != nil {
		rr.logger.Errorf("AppointmentRepo.FilterAppointmentsByPrice : Error getting appointments by price")
	...
	for cursor.Next(ctx) {
		...
		if err := cursor.Decode(&appointment); err != nil {
			rr.logger.Errorf("AppointmentRepo.FilterAppointmentsByPrice : Error decoding")
...
func (rr *AppointmentRepo) DeleteAppointmentsByAccommodationID(ctx context.Context, accommodationID string) error {
	...
	rr.logger.Infoln("AppointmentRepo.DeleteAppointmentsByAccommodationID : reached DeleteAppointmentsByAccommodationID in repo")
	...
	if err != nil {
		rr.logger.Errorf("AppointmentRepo.DeleteAppointmentsByAccommodationID : Error deleting appointments")
		span.SetStatus(codes.Error, "Error deleting appointments")
		return err
	}

	rr.logger.Printf("Deleted appointments count: ", result.DeletedCount)
	rr.logger.Infof("AppointmentRepo.DeleteAppointmentsByAccommodationID : Data of deleted appointment: %+v", accommodationID)
	rr.logger.Infoln("AppointmentRepo.DeleteAppointmentsByAccommodationID : DeleteAppointmentsByAccommodationID success")
...
func (rr *AppointmentRepo) GetAppointmentByID(ctx context.Context, id string) (*Appointment, error) {
	...
	rr.logger.Infoln("AppointmentRepo.GetAppointmentByID : reached GetAppointmentByID in repo")
	...
	if err != nil {
		rr.logger.Errorf("AppointmentRepo.GetAppointmentByID : Error getting appointment by ID")
...
func (rr *AppointmentRepo) GetAllAppointment(ctx context.Context) (*Appointments, error) {
	...
	rr.logger.Infoln("AppointmentRepo.GetAllAppointment : reached GetAllAppointment in repo")
	...
	if err != nil {
		rr.logger.Errorf("AppointmentRepo.GetAllAppointment : Error getting all appointments")
...
func (rr *AppointmentRepo) GetAppointmentsByAccommodation(ctx context.Context, id string) (Appointments, error) {
	...
	rr.logger.Infoln("AppointmentRepo.GetAppointmentsByAccommodation : reached GetAppointmentsByAccommodation in repo")

	appointmentsCollection := rr.getCollection()

	idValid, err := primitive.ObjectIDFromHex(id)
	if err != nil {
		rr.logger.Errorf("AppointmentRepo.GetAppointmentsByAccommodation : Error getting all appointments")
		span.SetStatus(codes.Error, "Error converting ID to ObjectID")
	...
	cursor, err := appointmentsCollection.Find(ctx, filter)
	if err != nil {
		rr.logger.Errorf("AppointmentRepo.GetAppointmentsByAccommodation : Error finding appointments by filter")
	...
	for cursor.Next(ctx) {
		var appointment Appointment
		if err := cursor.Decode(&appointment); err != nil {
			rr.logger.Errorf("AppointmentRepo.GetAppointmentsByAccommodation : Error decoding appointments")
...
func (rr *AppointmentRepo) GetAppointmentsByDate(ctx context.Context, startDate, endDate time.Time) (Appointments, error) {
	...
	rr.logger.Infoln("AppointmentRepo.GetAppointmentsByDate : reached GetAppointmentsByDate in repo")
	...
	cursor, err := appointmentsCollection.Find(ctx, filter)
	if err != nil {
		rr.logger.Errorf("AppointmentRepo.GetAppointmentsByDate : Error getting appointments")
		span.SetStatus(codes.Error, "Error getting appointments")
		return nil, err
	}
	defer cursor.Close(ctx)
	...
	for cursor.Next(ctx) {
		var appointment Appointment
		if err := cursor.Decode(&appointment); err != nil {
			rr.logger.Errorf("AppointmentRepo.GetAppointmentsByDate : Error decoding")
			span.SetStatus(codes.Error, "Error decoding")
			return nil, err
		}
	...
	if err := cursor.Err(); err != nil {
		rr.logger.Errorf("AppointmentRepo.GetAppointmentsByDate : Error getting appointments by date")
